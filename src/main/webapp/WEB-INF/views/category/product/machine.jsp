				...
				<div class="mainSection">
					<div class="listStyle">
						<ul class="listWrap">
							...
							<c:forEach var="productMachineList" items="${productMachineList}">
								<li class="listSet swiper-slide" data-product-id="156" data-price="32500">
									<div class="imgArea">
										<div class="thumbImg">
											<img data-src="../${productMachineList.imgList[0].img_url}" alt="게이샤 G1 워시드 에티오피아 썸네일 이미지" src="${productMachineList.imgList[0].img_url}">
										</div>
									</div>
									<div class="infoArea">
										<strong class="title">${productMachineList.product_name}</strong>
										<span class="subScript">${productMachineList.product_content}</span> 
										<span class="subScript">판매날짜 : <fmt:formatDate pattern = "yyyy.MM.dd" value="${productMachineList.product_date}"/></span> 
										<em class="price"><fmt:formatNumber value="${productMachineList.product_price}" pattern="#,### 원"/></em>
									</div> 
									<a href="productmaindetail?product_num=${productMachineList.product_num}" class="btnViewDetail"><span class="blind">상품 상세 보기</span></a>
								</li>
							</c:forEach>
						</ul>
					</div>
