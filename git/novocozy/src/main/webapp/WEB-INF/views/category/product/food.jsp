				...
				<div class="mainSection">
					<div class="listStyle">
						<ul class="listWrap">
							...
							<c:forEach var="productFoodList" items="${productFoodList}">
								<li class="listSet swiper-slide" data-product-id="156" data-price="32500">
									<div class="imgArea">
										<div class="thumbImg">
											<img data-src="../${productFoodList.imgList[0].img_url}" alt="게이샤 G1 워시드 에티오피아 썸네일 이미지" src="${productCoffeeList.imgList[0].img_url}">
										</div>
									</div>
									<div class="infoArea">
										<strong class="title">${productFoodList.product_name}</strong>
										<span class="subScript">${productFoodList.product_content}</span> 
										<span class="subScript">제작날짜 : <fmt:formatDate pattern = "yyyy.MM.dd" value="${productFoodList.product_date}"/></span> 
										<em class="price"><fmt:formatNumber value="${productFoodList.product_price}" pattern="#,### 원"/></em>
									</div> 
									<a href="productmaindetail?product_num=${productFoodList.product_num}" class="btnViewDetail"><span class="blind">상품 상세 보기</span></a>
								</li>
							</c:forEach>
						</ul>
					</div>
